#!/usr/bin/env python3
"""Paired multi-sample A/B for registered acBLAS Qwen3.5 decode linears."""
...
def main() -> int:
    ...
    if shape_counts != expected_shape_counts:
        raise RuntimeError(f"unexpected acBLAS patch counts: {shape_counts}")
    modules_by_name = dict(model._model.named_modules())
    patched_modules = [modules_by_name[name] for name in patched_names]
    ...
    def run_sample(sample, enabled: bool, pair_index: int) -> dict[str, object]:
        ...
        return {
            ...
            "mode": "acblas" if enabled else "optimized_baseline",
    ...
    # Warm both paths using the same sample before collecting paired records.
    ...
    for index, sample in enumerate(samples):
        ...
        acblas_records.append(pair[True])
        pair_ratios.append(
    ...
    acblas_accuracy = statistics.fmean(
        bool(record["correct"]) for record in acblas_records
    )
    payload = {
        ...
        "acblas": {
            "avg_ttft_ms": mean_metric(acblas_records, "ttft_ms"),
            "avg_throughput_tokens_per_sec": mean_metric(
                acblas_records, "throughput_tokens_per_sec"
            ),
            "accuracy": acblas_accuracy,
            "records": acblas_records,
        },
        "paired_decode": {
        ...
        "passed": (
            ...
            and baseline_accuracy == acblas_accuracy
        ),
    }
