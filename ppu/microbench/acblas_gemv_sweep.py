#!/usr/bin/env python3
"""Sweep acBLAS BF16 GEMV algorithms against the current torch.mv backend."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--matrix-copies", type=int, default=16)
...
def main() -> int:
    args = parse_args()
    library_path = args.library or Path(__file__).with_name("build") / "libseu_acblas_gemv.so"
    library = ctypes.CDLL(str(library_path.resolve()))
    launch = library.seu_acblas_linear_bf16
    launch.argtypes = [
        ctypes.c_void_p,
        ctypes.c_void_p,
        ctypes.c_void_p,
